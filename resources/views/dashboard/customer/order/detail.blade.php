<x-app-layout>
    <x-slot name="header_content">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a><i class="fa-solid fa-circle-info"></i> {{ __('Detail Pesanan Saya') }}</a></li>
        </ol>
    </x-slot>
                    ...
                    @foreach( $cust_orders as $item )
                        <div class="col-xl-12 col-sm-12 col-md-12 col-lg-12 text-center">
                            ...
                            <div class="form-group">
                                ...
                                <input disabled value="Menunggu konfirmasi admin." id="status" name="status" class="form-control text-center bg-warning text-white" required="">
                            </div>
                            @elseif($item -> status == 1)
                            <div class="form-group">
                                <label hidden for="status">Status</label>
                                <input disabled value="Pesanan Dikonfirmasi, Barang akan segera dikirim." id="status" name="status" class="form-control text-center bg-success text-white" required="">
                            </div>
                            @elseif($item -> status == 2)
                                <div class="form-group">
                                    <label hidden for="status">Status</label>
                                    <input disabled value="Maaf pesanan anda kami tolak, Dana akan segera dikembalikan jika anda sudah membayar tagihan." id="status" name="status" class="form-control text-center bg-danger text-white" required="">
                                </div>
                            @endif
                        </div>
